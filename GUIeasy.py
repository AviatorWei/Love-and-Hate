...
'''
DB = {}
with open('value4.json', 'r', encoding='utf-8') as f:
    DB = json.load(f)
'''
...
def PerfectGo(pos):
    ...
    print('---')
    print(DB[str(root[pos])])
    value = DB[str(root[pos])][0]
    remoteness = DB[str(root[pos])][1]
    for m in moves:
        nextMove = DoMove(pos, m)
        print(DB[str(root[pos])])
        if DB[str(root[nextMove])][1] == remoteness - 1 and DB[str(root[nextMove])][0] != value:
            ClickMaker(m)(1)
            break
